...
def login():
    ...
    if request.method == "GET":
        return render_template("log-in.html", username='JoyceChan', password="temp")
        ...
        if result:
            ...
            if result ['password'] == f['password']:
                ...
                return render_template("log-in.html", username='joyce@marsden.com', password="temp", error=error)
        return render_template("index.html")
...
def member():
    # query for the page
    sql = """select member.member_id, member.firstname, member.secondname, member.password, member.username, member.authorisation
           from member
           """
